...
class SEBlock(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc3 = nn.Linear(in_channels, in_channels // reduction_ratio)  # Add an additional fully connected layer
        self.fc4 = nn.Linear(in_channels // reduction_ratio, in_channels)  # Add an additional fully connected layer
...
class SE_ResNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.se_block = SEBlock(num_features)  # Add SE(Block)
...
# model = ResNetWithAttention(num_classes).to(device)

# Load the ResNet model
num_classes = len(train_dataset.classes)
model = SE_ResNet(num_classes).to(device)
...
torch.save(model.state_dict(), "SE_ResNet_50_saved_model.pth")
